...
st.set_page_config(page_title="Badminton Player Rotation", layout="centered")
st.title("🏸 Badminton Player Rotation App")
...
# Rotation logic
def rotate_players(active_players, num_courts):
    total = len(active_players)
    if total < 3:
        st.warning("At least 3 active players are needed to start rotation.")
        return [], [], []

    if st.session_state.rotation_count == 1:
        random.shuffle(active_players)
        st.session_state.bench_queue = active_players.copy()

    selected = []
    courts = []
    court_index = 0
    queue = st.session_state.bench_queue.copy()

    while court_index < num_courts and len(queue) >= 2:
        if len(queue) >= 4:
            court_players = queue[:4]
            queue = queue[4:]
        elif len(queue) == 3:
            court_players = queue[:2]
            queue = queue[2:]
        else:
            break
    ...
    st.session_state.bench_queue = queue + selected
    bench = [p for p in active_players if p not in selected]
    return courts, bench, selected
...
if present_players:
	...
	# Recalculate active players before each rotation
        active_players = [p for p in present_players if p not in st.session_state.rest_players]

        st.session_state.rotation_count += 1
        courts, bench, selected = rotate_players(active_players, num_courts)
